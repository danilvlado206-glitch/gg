...
from dsm.skill import ReasoningTrace, SkillCrystallizer, SkillKernel, SkillRouteResult, route_skill_kernels
...
class DynamicSegmentedMemory:
    ...
    def __init__(
        ...
        self.skill_kernels: dict[str, SkillKernel] = {}
        self.categories = CategoryTree(self.embedding_model)
        self.graph = MemoryGraph()
        self.index = SegmentIndex()
        self.skill_crystallizer = SkillCrystallizer(self.embedding_model)
    ...
    def active_context(
        ...
        *,
        ...
        selected_skills = self.route_skills(query, k=skill_k) if skill_k > 0 else []
        used = 0
        bounded_skills = []
        for item in selected_skills:
            cost = item.kernel.estimated_tokens
            if bounded_skills and used + cost > budget:
                continue
            bounded_skills.append(item)
            used += cost
        selected_skills = bounded_skills
        ...
        for index, item in enumerate(selected_skills, start=1):
            kernel = item.kernel
            path = " → ".join(kernel.category_path)
            parts.append(
                "\n".join(
                    [
                        f"[SKILL KERNEL {index}] id={kernel.id}",
                        f"category={path}",
    ...
    def crystallize_skill(
        ...
        *,
        name: str | None = None,
        metadata: dict[str, Any] | None = None,
        category_path: tuple[str, ...] | list[str] | str = ("Skills", "Crystallized"),
        integrate_as_segment: bool = True,
    ) -> SkillKernel:
        ...
        kernel = self.skill_crystallizer.crystallize(
            trace,
            name=name,
            category_path=normalize_path(category_path),
        )

        existing = self._find_existing_skill(kernel)
        if existing:
            existing.reinforce(outcome)
            existing.metadata.update(metadata or {})
            return existing

        if integrate_as_segment:
            source_segments = self.write(
                kernel.to_segment_text(),
                description=kernel.description,
                category_path=kernel.category_path,
                metadata={"type": "skill_kernel", "skill_kernel_id": kernel.id, **(metadata or {})},
                importance=kernel.priorities.importance,
                update_existing=False,
            )
            kernel.source_segment_ids = tuple(segment.id for segment in source_segments)

        self.skill_kernels[kernel.id] = kernel
        return kernel
        ...
        self.storage.save(
            {
                ...
                "skill_kernels": [kernel.to_dict() for kernel in self.skill_kernels.values()],
    ...
    def load(self) -> None:
        ...
        self.skill_kernels = {
            kernel.id: kernel
            for kernel in (
                SkillKernel.from_dict(item) for item in data.get("skill_kernels", [])
            )
        }
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "skill_kernels": len(self.skill_kernels),
    ...
    def _find_existing_skill(self, kernel: SkillKernel, threshold: float = 0.88) -> SkillKernel | None:
        for existing in self.skill_kernels.values():
            same_name = existing.name.lower() == kernel.name.lower()
            if same_name or cosine(existing.embedding, kernel.embedding) >= threshold:
                return existing
        return None
    ...
    def _global_summary(self, selected: list[RouteResult]) -> str:
        if not selected:
            if self.skill_kernels:
                return "No active memory segments selected; procedural skill kernels remain available."
