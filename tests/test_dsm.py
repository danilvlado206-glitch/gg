...
def test_skill_crystallization_and_retrieval(tmp_path: Path) -> None:
    memory = DynamicSegmentedMemory(tmp_path / "dsm.json")
    ...
    active = memory.active_context("Rust websocket cancellation backpressure", k=1)

    assert skills[0].kernel.id == kernel.id
    assert kernel.id in active.skill_ids
    assert "Skill Kernel" in active.context_text
    assert memory.stats()["skill_kernels"] == 1


def test_skill_persistence_roundtrip(tmp_path: Path) -> None:
    path = tmp_path / "dsm.json"
    memory = DynamicSegmentedMemory(path)
    kernel = memory.crystallize_skill(
        "Plan a persistent cognition feature",
        ["Capture the successful trace.", "Extract reusable planning steps."],
        "A reusable planning kernel is available for future retrieval.",
    )
    memory.save()
